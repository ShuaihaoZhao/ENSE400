Shader "NatureManufacture Shaders/Standard Shaders/Standard Metalic Snow CutOut"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float4 tex2DNode486 = tex2D( _DetailMapAlbedoRNyGNxA, uv_DetailMapAlbedoRNyGNxA );
			...
			float3 normalizeResult483 = normalize( BlendNormals( UnpackScaleNormal( tex2D( _BumpMap, uv_MainTex ), _SnowNormalCoverHardness ) , temp_output_504_0 ) );
			...
			float clampResult502 = clamp( ( tex2D( _SnowMaskB, uv_SnowMaskB ).b * _SnowMaskPower ) , 0.0 , 1.0 );
