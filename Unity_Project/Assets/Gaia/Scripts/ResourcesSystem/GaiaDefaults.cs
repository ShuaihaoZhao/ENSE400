    ...
    public class GaiaDefaults : ScriptableObject
    {
        ...
        private void CreateTile(int tx, int tz, ref Terrain[,] world, GaiaResource resources)
        {
            ...
            if (rendererType == GaiaConstants.EnvironmentRenderer.BuiltIn)
            {
                ...
                {
#if UNITY_EDITOR
                    GaiaPipelineUtils.SetupPipeline(rendererType, null, null, null, "Procedural Worlds/Simple Water");
#endif
                }
            }
            else
            {
                ...
                if (rendererType == GaiaConstants.EnvironmentRenderer.LightWeight2018x)
                {
                    ...
                    GaiaPipelineUtils.SetupPipeline(rendererType, "Procedural Worlds Lightweight Pipeline Profile", "Pipeline Terrain Material", "Lightweight Render Pipeline/Terrain/Lit", "Procedural Worlds/Simple Water LW");
#else
                ...
                {
#if UNITY_EDITOR && UNITY_2018_3_OR_NEWER 
                    GaiaPipelineUtils.SetupPipeline(rendererType, "Procedural Worlds HDRenderPipelineAsset", "Pipeline Terrain Material", "HDRP/TerrainLit", "Procedural Worlds/Simple Water HD");
#else
                    Debug.LogWarning("Lightweight Pipeline is only supposted in 2018.3 or newer");
#endif
                }
